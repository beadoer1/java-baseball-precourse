...
- 같은 수가 같은 자리에 있으면 스트라이크, 다른 자리에 있으면 볼, 같은 수가 전혀 없으면 포볼 또는 낫싱이란 힌트를 얻고, 그 힌트를 이용해서 먼저 상대방(컴퓨터)의 수를 맞추면 승리한다.
    - 예) 상대방(컴퓨터)의 수가 426일 때, 123을 제시한 경우 : 1 스트라이크, 456을 제시한 경우 : 1스트라이크 1볼, 789를 제시한 경우 : 낫싱
...
3. BaseballNumber
    1. 속성
        ...
        - Number number
    2. 메서드
        - Check result(BaseballNumber anotherBaseballNumber)

4. BaseballNumbers
    1. 속성
        - List<BaseballNumber> baseballNumbers
    2. 메서드
        - ResultDto result(BaseballNumber anotherBaseballNumber)
